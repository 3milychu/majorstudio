...
function preload (){
	table = loadTable("data/selected_mediums_MetObjects.csv","csv","header");
...
d3.csv("/data/selected_mediums_MetObjects.csv", function(data) {
...
// Create dictionary of objects with key: year; value: number of objects in year
			...
			var countByYear = d3.nest()
			  ...
			  .entries(data);
			console.log(JSON.stringify(countByYear));
...
// Create histogram
			...
			// set the dimensions and margins of the graph
				...
				// get the data
				d3.csv("/data/selected_mediums_MetObjects.csv", function(error, data) {
				  ...
				  data.forEach(function(d) {
					...
					for (var i=0; i<d.objectBeginDate.length; i++){
						if (d.objectBeginDate[i] < 0) {
							d.objectBeginDate = (abs(countyByYear.key[i])  + " BCE");
							} else {
							d.objectBeginDate = d.objectBeginDate[i];
							}
						}
						console.log(d.objectBeginDate);
